    ...
    while True:
        ...
        try:
            if("tiul" in chunk.decode() and "1.0," not in chunk.decode()):
                ...
                textarea.insert(END,"\n"+chunk.decode('ascii'))
                textarea.see("end")
            elif(chunk.decode() == "Womp Womp. The client declined your request..."):
                labelchat.configure(text="")
                textarea.insert(END,"\n"+chunk.decode('ascii'))
                textarea.see("end")
...
def browseFiles():
    ...
    try:
        ...
        if(message[:4] == "send"):
            ...
            file_size = getFileSize("shared_songs/"+sending_file)
...
def sendMessage():
    ...
    if(msgtosend == "y" or msgtosend == "Y"):
        ...
        textarea.insert(END,"\n"+"\nPlease wait, song is downloading.....")
        textarea.see("end")
        HOSTNAME = "127.0.0.1"
        USERNAME = "lftpd"
        PASSWORD = "lftpd"
        ...
        textarea.insert(END,"\n"+"Song successfully downloaded to path:"+download_path)
        textarea.see("end")
...
def openChatWindow():
    ...
    connectserver = Button(window,text="Connect to Music Server",bd=1, font = ("Calibri",10), command = connectToServer)
